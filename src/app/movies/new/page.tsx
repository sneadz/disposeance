...
import { Search, Plus, Calendar, Star, Check, ChevronLeft, Film } from 'lucide-react'
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch('/api/movies', {
        ...
        body: JSON.stringify({ movie: selectedMovie, availableDates: selectedDates }),
      })
      ...
      <main className="min-h-screen bg-zinc-950 text-white">
        <header className="sticky top-0 z-10 bg-zinc-950/80 backdrop-blur-md border-b border-zinc-800/60 px-4 py-3">
          <div className="max-w-2xl mx-auto flex items-center gap-3">
            <button onClick={() => setSelectedMovie(null)} className="p-1.5 text-zinc-400 active:text-white transition-colors">
        ...
        <div className="max-w-2xl mx-auto px-4 py-6 space-y-5">
          ...
          >
            {submitting ? 'Création...' : `Lancer le vote — ${selectedDates.length} jour${selectedDates.length > 1 ? 's' : ''}`}
          </button>
        </div>
      </main>
    )
